...
import { getDesiredNsmVersion, getMeta, setDesiredNsmVersion, setMeta } from '@/persistence/clusterMetaPersistence';
...
describe('nodePersistence', () => {
    it('upserts, lists and deletes registry nodes', async () => {
        await upsertNodeModel({ nodeId: 'n1', address: '1.1.1.1', apiPort: 2, lastSeen: 1, isLeader: false });
        await upsertNodeModel({ nodeId: 'n1', address: '2.2.2.2', apiPort: 2, lastSeen: 2, isLeader: false });
    ...
    it('touchNodeModel upserts current address + stamps lastSeen (IP change propagation)', async () => {
        await touchNodeModel('n2', '10.0.0.1', 1025, true);
        const first = await getNodeByIdModel('n2');
        expect(first?.address).toBe('10.0.0.1');
        expect(first?.isLeader).toBe(true);
        expect(first?.lastSeen).toBeGreaterThan(0);
        await touchNodeModel('n2', '10.0.0.2', 1025, true);
        expect((await getNodeByIdModel('n2'))?.address).toBe('10.0.0.2');
    });
});
...
describe('clusterMetaPersistence', () => {
    ...
    it('tracks the desired NSM version', async () => {
