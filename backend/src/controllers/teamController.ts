...
export const redactProjectSecrets = (project: Project, canConfigure: boolean): Project => {
    // Secrets stored only as hashes (deploy key) and the GitHub webhook token ref are never needed by
    // the client; strip them from every response. The one-time raw deploy key is returned separately.
    const base = { ...project, deploymentKey: undefined, deploymentKeyHash: undefined, githubWebhook: undefined };
    if (canConfigure) return base;
    return { ...base, secrets: project.secrets?.map((s) => ({ ...s, secretValue: s.variable ? s.secretValue : '' })) };
};
