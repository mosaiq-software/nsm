...
vi.mock('@/cluster/membership', () => ({ handleJoin: vi.fn(async () => ({ ok: true })), handleRemove: vi.fn(async () => ({ ok: true })) }));
...
describe('internal cluster-secret routes', () => {
    ...
    it('/cluster/join is leader-gated (follower forwards)', async () => {
        isLeader.mockReturnValue(false);
        const res = await request(app).post('/cluster/join').set(SECRET, config.clusterSecret).send({});
        expect(res.status).toBe(599);
    });
    it('/cluster/join succeeds on the leader', async () => {
        const res = await request(app).post('/cluster/join').set(SECRET, config.clusterSecret).send({ node: {}, token: config.clusterSecret });
        expect(res.status).toBe(200);
