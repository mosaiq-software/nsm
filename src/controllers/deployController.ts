...
import { DEFAULT_TIMEOUT, NSM_LABEL_SERVICE_INSTANCE_ID } from '@/constants';
...
export const deployProject = async (projectId: string): Promise<string | undefined> => {
    ...
    try {
        ...
        const { conf: nginxConf, domains } = getNginxConf(project, requestedPorts, plan.dirs, assignedNode.address);
...
const getNginxConf = (project: Project, requestedPorts: PortPlanEntry[], ensuredDirs: FullDirectoryMap, nodeAddress: string): { conf: string; domains: string[] } => {
    ...
    for (const server of projectDC.nginxConfig?.servers || []) {
        ...
        for (const location of server.locations) {
            if (location.type === NginxConfigLocationType.PROXY) {
                ...
                if (req) (location as ProxyConfigLocation).proxyPass = `http://${nodeAddress}:${req.port}`;
            }
            if (location.type === NginxConfigLocationType.STATIC) {
