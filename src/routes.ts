...
privateRouter.use(async (req, res, next) => {
    ...
    if (!(await verifyAuthToken(authHeader))) {
...
// === Public ===
publicRouter.get('/', async (_req, res) => res.status(200).send('Hello from the NSM API'));
...
internalRouter.post('/cluster/join', requireClusterSecret, async (req, res) => {
    if (!requireLeader(req, res)) return;
    res.status(200).json(await handleJoin(req.body));
});

internalRouter.post('/cluster/remove', requireClusterSecret, async (req, res) => {
    if (!requireLeader(req, res)) return;
    res.status(200).json(await handleRemove(req.body?.nodeId));
});
