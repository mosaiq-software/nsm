...
import { upsertCertModel } from '@/persistence/certPersistence';
import { deleteNodeModel, upsertNodeModel } from '@/persistence/nodePersistence';
import { setDesiredNsmVersion, setLastAppliedIndex } from '@/persistence/clusterMetaPersistence';
...
// Deterministic, idempotent application of a committed op onto the local SQLite view.
// Keyed by raft log index via setLastAppliedIndex so replays are safe.
export const applyOp = async (op: Op, index: number): Promise<void> => {
    switch (op.type) {
        ...
        case OpType.CLEAR_DESIRED_DEPLOYMENT:
            await deleteDesiredDeploymentModel(op.projectId);
            break;
        case OpType.UPSERT_CERT:
            await upsertCertModel(op.cert);
            break;
        case OpType.UPSERT_NODE:
            await upsertNodeModel({ ...op.node, voter: true });
            break;
        case OpType.REMOVE_NODE:
            await deleteNodeModel(op.nodeId);
            break;
    ...
    await setLastAppliedIndex(index);
};
